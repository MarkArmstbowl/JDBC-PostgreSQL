...
<%
    ...
    if (action != null && action.equals("INSERT INTO DATABASE")) {
        ...
        PreparedStatement undergrad = conn.prepareStatement(
        ("INSERT INTO STUDENT_UNDERGRAD VALUES (?, ?)"));
        ...
        for(int i = 0; i < majorS.length; i++) {
            major.setString(1, request.getParameter("student_ID"));
            major.setString(2, majorS[i]);
            major.executeUpdate();
        }
        major.close();

        String[] minorS = request.getParameterValues("minorS");

        for(int i = 0; i < minorS.length; i++) {
            minor.setString(1, request.getParameter("student_ID"));
            minor.setString(2, minorS[i]);
            minor.executeUpdate();
        }
        minor.close();


        undergrad.setString(1, request.getParameter("student_ID"));
        undergrad.setString(2, request.getParameter("college"));
        undergrad.executeUpdate();
        undergrad.close();
